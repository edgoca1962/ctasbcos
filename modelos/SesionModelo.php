...
class SesionModelo extends BaseLibreria
{
    ...
    protected function get_usuario_sesion_modelo()
    {
        ...
        $this->consulta_base_datos->bindParam(":id_usuario", $this->id_usuario, PDO::PARAM_STR);
    ...
    {
        $this->consulta_base_datos = $this->conectar()->prepare("INSERT INTO sesiones (id_usuario,id_sesion,fecha_inicio,hora_inicio,estado_sesion) 
        VALUES(:id_usuario,:id_sesion,:fecha_inicio,:hora_inicio,:estado_sesion)");
        $this->consulta_base_datos->bindParam(':id_sesion', $this->id_sesion, PDO::PARAM_STR);
        $this->consulta_base_datos->bindParam(':id_usuario', $this->id_usuario, PDO::PARAM_STR);
        $this->consulta_base_datos->bindParam(':fecha_inicio', $this->fecha_inicio, PDO::PARAM_STR);
        $this->consulta_base_datos->bindParam(':hora_inicio', $this->hora_inicio, PDO::PARAM_STR);
        $this->consulta_base_datos->bindParam(':estado_sesion', $this->estado_sesion, PDO::PARAM_STR);
    ...
    protected function get_datos_sesion_modelo()
    {
        ...
        $this->consulta_base_datos->bindParam(":id_sesion", $this->id_sesion);
        $this->consulta_base_datos->execute();
        $this->datos_sesion = $this->consulta_base_datos->fetch();
        $this->conexion = null;
        if (empty($this->consulta_base_datos)) {
            $this->conexion = null;
            return false;
        } else {
    ...
    protected function cierra_sesion_modelo()
    {
        ...
        $this->consulta_base_datos = $this->conectar()->prepare("UPDATE sesiones SET fecha_salida = :fecha_salida, hora_salida = :hora_salida, estado_sesion = :estado_sesion WHERE id_sesion = :id_sesion");
        $this->consulta_base_datos->bindParam(':fecha_salida', $this->fecha_salida, PDO::PARAM_STR);
        $this->consulta_base_datos->bindParam(':hora_salida', $this->hora_salida, PDO::PARAM_STR);
        $this->consulta_base_datos->bindParam(':estado_sesion', $this->estado_sesion, PDO::PARAM_STR);
        $this->consulta_base_datos->bindParam(':id_sesion', $this->id_sesion, PDO::PARAM_STR);
        if ($this->consulta_base_datos->execute()) {
            $this->conexion = null;
    ...
    protected function cierra_sesion_ajax_modelo()
    {
        ...
        $this->hora_salida = date("H:i:s");
        $this->estado_sesion = 0;
        $this->consulta_base_datos = $this->conectar()->prepare("UPDATE sesiones SET fecha_salida = :fecha_salida, hora_salida = :hora_salida, estado_sesion = :estado_sesion WHERE $this->campo = :valor AND estado_sesion = 1");
        $this->consulta_base_datos->bindParam(':fecha_salida', $this->fecha_salida, PDO::PARAM_STR);
        $this->consulta_base_datos->bindParam(':hora_salida', $this->hora_salida, PDO::PARAM_STR);
        $this->consulta_base_datos->bindParam(':estado_sesion', $this->estado_sesion, PDO::PARAM_STR);
        $this->consulta_base_datos->bindParam(':valor', $this->valor, PDO::PARAM_STR);
        if ($this->consulta_base_datos->execute()) {
            $this->conexion = null;
            return true;
        } else {
    ...
    {
        $this->consulta_base_datos = $this->conectar()->prepare("DELETE FROM sesiones WHERE id = :id");
        $this->consulta_base_datos->bindParam(':id', $this->id_sesion, PDO::PARAM_STR);
        if ($this->consulta_base_datos->execute()) {
            $this->conexion = null;
            return true;
        } else {
